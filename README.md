...
- Growth-first, N<sub>2O-consumption-second optimization.
...
It tells SynComDesign which model IDs correspond to nitrate, nitrite, NO, N<sub>2O, and N<sub>2<sub>.
...
If your model uses a different N<sub>2O exchange reaction or metabolite ID, add it to this table.
...
### ID5: Growth-Then-N<sub>2O-Consumption
...
3. Within that growth constraint, maximize N<sub>2O uptake.

If the models cannot consume N<sub>2O under the current medium, ID5 may give the same result as ID1.
...
- `n2o_net_flux`: raw N<sub>2O exchange direction.
...
For example, if the COBRA exchange flux through an N<sub>2O exchange reaction is `-3`, SynComDesign reports:
...
### 14.4 N<sub>2O Uptake Is Always Zero

Possible causes:

- The model has no N<sub>2O exchange reaction.
- `metabolite_aliases.tsv` does not include the correct N<sub>2O ID.
- The medium does not allow N<sub>2O uptake.
- The model has no pathway for using N<sub>2O.
